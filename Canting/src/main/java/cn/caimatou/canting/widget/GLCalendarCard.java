 ...
 * <br/><br/>
 */
public class GLCalendarCard extends View {
    ...
    public GLCalendarCard(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context);
    }

    public GLCalendarCard(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public GLCalendarCard(Context context) {
        super(context);
        init(context);
    }

    public GLCalendarCard(Context context, OnCellClickListener listener) {
        super(context);
    ...
    private void fillDate() {
        int monthDay = GLDateUtil.getCurrentMonthDay(); // 今天
        int lastMonthDays = GLDateUtil.getMonthDays(mShowDate.year, mShowDate.month - 1); // 上个月的天数
        int currentMonthDays = GLDateUtil.getMonthDays(mShowDate.year, mShowDate.month); // 当前月的天数
        int firstDayWeek = GLDateUtil.getWeekDayFromDate(mShowDate.year, mShowDate.month);
        boolean isCurrentMonth = false;
        if (GLDateUtil.isCurrentMonth(mShowDate)) {
            isCurrentMonth = true;
        }
        int day = 0;
    ...
    class Cell {
        ...
        public void drawSelf(Canvas canvas) {
            switch (state) {
                case TODAY: // 今天
                    mTextPaint.setColor(GLResourcesUtil.getColor(R.color.white));
                    canvas.drawCircle((float) (mCellSpace * (i + 0.5)),
                            (float) ((j + 0.5) * mCellSpace), mCellSpace / 3, mCirclePaint);
                    break;
                case CURRENT_MONTH_DAY: // 当前月日期
                    mTextPaint.setColor(GLResourcesUtil.getColor(R.color.gray4));
                    break;
                case PAST_MONTH_DAY: // 过去一个月
                    mTextPaint.setColor(GLResourcesUtil.getColor(R.color.gray1));
                    break;
