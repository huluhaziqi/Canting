package cn.caimatou.canting.threadpool;
...
public class GLThreadPool {
    ...
    class GLMessageHandler<T> extends Handler {
        ...
        public GLMessageHandler(Looper looper, GLFuture<T> future, GLFutureListener<T> listener) {
            super(looper);
            this.mFuture = future;
    ...
    public GLThreadPool() {
        mExecutor = new ThreadPoolExecutor(
                ...
                new GLPriorityThreadFactory("canting-thread-pool", android.os.Process.THREAD_PRIORITY_DEFAULT));
    }
    ...
    private class ChatWorker<T> implements Runnable, GLFuture<T> {
        ...
        public ChatWorker(Job<T> job, GLFutureListener<T> listener) {
            this.mJob = job;
            if (null != listener) {
                this.mHandler = new GLMessageHandler(GLCantingApp.getIns().getMainLooper(), this, listener);
            }
        }
